...
interface CartItem {
    ...
    quantity: number;
    total_price: string;
    product_image: string; // This path is like 'uploads/products/image.jpg'
}
...
// ✅ FIX 1: Removed trailing slash for consistency
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    ...
                    {!isLoading && !error && (!cart || cart.items.length === 0) ? (
                        <div className="text-center py-20 bg-white rounded-lg shadow-md">
                            ...
                            <Link href="/product" className="mt-6 inline-block px-8 py-3 text-white font-semibold bg-gray-800 rounded-lg hover:bg-black transition-transform transform hover:scale-105">
                                Start Shopping
                            </Link>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-2 bg-white p-6 rounded-lg shadow-md relative">
                                {isLoading && <div className="absolute inset-0 bg-white/50 flex items-center justify-center"><LoaderIcon/></div>}
                                <ul role="list" className="divide-y divide-gray-200">
                                    {cart?.items.map((item) => (
                                        <li key={item.id} className="flex py-6">
                                            <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                                                <img
                                                    // ✅ FIX 2: Added the '/media/' prefix to construct the correct URL
                                                    src={`${API_BASE_URL}/media/${item.product_image}`}
                                            ...
                                            <div className="ml-4 flex flex-1 flex-col">
                                                ...
                                                <div className="flex flex-1 items-end justify-between text-sm">
                                                    ...
                                                    <div className="flex">
                                                        <button onClick={() => handleRemoveItem(item.id)} type="button" className="font-medium text-amber-600 hover:text-amber-500 flex items-center gap-1">
                                                            <TrashIcon /> Remove
                                                        </button>
                                                    </div>
                                                </div>
                            ...
                            <div className="lg:col-span-1">
                                <div className="bg-white p-6 rounded-lg shadow-md sticky top-24">
                                    ...
                                    <button className="mt-6 w-full bg-gray-800 text-white font-bold py-3 rounded-lg hover:bg-black transition-transform transform hover:scale-105">
                                        Proceed to Checkout
                                    </button>
                                </div>
                            </div>
